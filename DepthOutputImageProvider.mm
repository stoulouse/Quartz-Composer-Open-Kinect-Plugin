...
- (BOOL) renderToBuffer:(void*)baseAddress
	...
	if (pthread_mutex_trylock(&_plugin->_depthbackbuf_mutex)) {
		if (_plugin->_got_depth) {
			uint8_t* tmp = _plugin->_depth_front;
			_plugin->_depth_front = _plugin->_depth_mid;
			_plugin->_depth_mid = tmp;
			_plugin->_got_depth = 0;
		}

		pthread_mutex_unlock(&_plugin->_depthbackbuf_mutex);
	}
	
	if (_plugin->_depth_front) {
		uint8_t* buf = (uint8_t*)baseAddress;
		memcpy(buf, _plugin->_depth_front, bounds.size.height * bounds.size.width * 4);
		/*
		if (format == QCPlugInPixelFormatARGB8) {
			...
			for (int i = 0; i < bounds.size.height; ++i) {
				for (int j = 0; j < bounds.size.width; ++j) {
					int idx = j * 4 + i * (int)bounds.size.width * 4;
					int bufIdx = j * 4 + i * (int)bounds.size.width * 4;
					buf[bufIdx + 0] = _plugin->_depth_front[idx + 3];
					buf[bufIdx + 1] = _plugin->_depth_front[idx + 2];
					buf[bufIdx + 2] = _plugin->_depth_front[idx + 1];
					buf[bufIdx + 3] = _plugin->_depth_front[idx + 0];
				}
			}
		}*/
	}
